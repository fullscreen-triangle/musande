...
The five publication panels visualize the validation:
\begin{itemize}[leftmargin=*]
\item Panel 1: receiver floor and cell-truth (\Cref{fig:panel1}).
\item Panel 2: representational invariance and triple equivalence
(\Cref{fig:panel2}).
\item Panel 3: layered receivers and mode non-privilege
(\Cref{fig:panel3}).
\item Panel 4: methodological floor and catalytic composition
(\Cref{fig:panel4}).
\item Panel 5: mode--methodology equivalence and distribution
(\Cref{fig:panel5}).
\end{itemize}

\begin{figure}[h!]\centering
\includegraphics[width=0.95\textwidth]{validation/figures/panel_1.png}
\caption{\textbf{Receiver floor and cell-truth.}
(a)~Distribution of $S$-values across $10^4$ random states for a fixed
receiver, showing the positive floor $\beta$.
(b)~Cell-truth verification: $S$ is constant inside the action-cell.
(c)~Floor attainment as a function of decoder noise $\beta$.
(d)~3D surface of $S(\receiver,x;\Cell)$ over a 2D state slice and
varying $\beta$.}
\label{fig:panel1}
\end{figure}

\begin{figure}[h!]\centering
\includegraphics[width=0.95\textwidth]{validation/figures/panel_2.png}
\caption{\textbf{Representational invariance.}
(a)~$S$-values under oscillatory encoding vs.\ original.
(b)~Same under categorical encoding.
(c)~Same under partition encoding.
(d)~3D manifold of $S$ over $(\text{representation},\text{state})$ showing
the level structure preserved across encodings.}
\label{fig:panel2}
\end{figure}

\begin{figure}[h!]\centering
\includegraphics[width=0.95\textwidth]{validation/figures/panel_3.png}
\caption{\textbf{Layered receivers and mode non-privilege.}
(a)~Per-layer floors $\beta_i$ and aggregate floor.
(b)~Pre-decoder firing region map.
(c)~Fraction of states reachable through reflex only vs.\ decoder only.
(d)~3D heatmap of $S(\receiver_\bullet,x;\Cell)$ vs.\ layer floors.}
\label{fig:panel3}
\end{figure}

\begin{figure}[h!]\centering
\includegraphics[width=0.95\textwidth]{validation/figures/panel_4.png}
\caption{\textbf{Methodological floor.}
(a)~$S$-trajectory across $N$ iterations approaching
$\Sfloor(\Method)=\sigma\kappa/(1-\kappa)$.
(b)~Floor as a function of $\kappa$ for fixed $\sigma$.
(c)~Floor as a function of $\sigma$ for fixed $\kappa$.
(d)~3D surface of $\Sfloor(\Method)$ over $(\kappa,\sigma)$.}
\label{fig:panel4}
\end{figure}

\begin{figure}[h!]\centering
\includegraphics[width=0.95\textwidth]{validation/figures/panel_5.png}
\caption{\textbf{Mode--methodology equivalence and distribution.}
(a)~Composite floor $\Sfloor(\receiver\circ\Method)$ measured vs.\
predicted closed-form.
(b)~Multiplicative composition under stacking $n$ methodologies.
(c)~Anti-monopoly: $\sup_{x\in X_0}\mathrm{Know}_\receiver(x)$ as a
function of $\Sfloor(\receiver)$.
(d)~3D surface of composite floor over $(\Sfloor(\receiver),\Sfloor(\Method))$.}
\label{fig:panel5}
\end{figure}
